...
const CreateExtractor = ({}) => {
  ...
  const [lastValidSchema, setLastValidSchema] = React.useState(JSON.parse(startSchema));
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    mutate({ name, description: shortDescription, schema: objectSchema, instruction });
  };
  ...
  const handleSuggest = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    console.log(`Making request with description: ${description} and schema: ${schema}`);
    ...
    <div className="w-4/5 m-auto">
      ...
      <form className="m-auto flex flex gap-2 mt-5" onSubmit={handleSuggest}>
        ...
        {suggestMutation.isPending ? (
          ...
          <IconButton type="submit" icon={<ArrowUpIconImported />} aria-label="OK" colorScheme="blue" />
        )}
      </form>
      <form className="m-auto flex flex-col content-between gap-5 mt-10" onSubmit={handleSubmit}>
          ...
          <AccordionItem>
            <AccordionButton>
              ...
              <div className="ml-auto">
                {currentSchemaValid ? <Badge colorScheme="green">OK</Badge> : <Badge colorScheme="red">Errors!</Badge>}
                <AccordionIcon />
              </div>
            </AccordionButton>
            <AccordionPanel>
        ...
        {Object.keys(lastValidSchema).length !== 0 && (
          <>
            ...
            {!currentSchemaValid && (
              <Text color="red.500">JSON Schema has errors. Showing previous valid JSON Schema.</Text>
            )}
            <Card>
              <CardBody>
                <Form
